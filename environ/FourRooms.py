...
class FourRooms(gym.Env):
    ...
    def __init__(self):
        ...
        self.bump_reward = -0.01

        # start state random location in start room
        start_room = 0
        sz = self.room_sizes[start_room]
        self.start_state = self.offsets[start_room] + np.random.randint(sz[0] * sz[1])
        self._reset()
    ...
    def _step(self, action):
        ...
        if self.state == self.terminal_state:
            ...
            self.state_raw = self.goal
            self.done = True
            return self.state_raw, self._get_reward(), self.done, self.state

        in_hallway = self.in_hallway_index()
        # print("in hallway:",in_hallway)
        ...
        self.new_state = self.encode([room2, coord2])
        self.new_state_raw = [room2,coord2]
        ...
        reward = self._get_reward(new_state=self.new_state)

        return self.new_state_raw, reward, self.done, self.new_state
...
register(id='FourRooms-v0',entry_point='environ.FourRooms:FourRooms',timestep_limit=1000000000)
