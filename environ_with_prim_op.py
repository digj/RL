...
def execute_option(option,state,env):
    reward = 0
    i = 0
    if option == 8:
        action = UP
        ...
        i = 1
        return rew, next_ob_enc, next_ob, i, done
    if option == 9:
        action = DOWN
        next_ob, rew, done, next_ob_enc = env.step(action)
        i = 1
    ...
    while True:
        if option == 0:#hall way to room 2
            ...
            if done:
                ...
                return reward, next_ob_enc, next_ob, i, done
            print(next_ob)
            state = next_ob
            reward = reward + rew*gamma**(i)
...
def main():
    ...
    while True:
        ...
        while not done:
            ...
            # print("option: ",option)
            reward, next_ob_enc, next_ob,j, done = execute_option(option, ob,env)
            print("obenc:", ob_enc)
            print("nextobenc:",next_ob_enc)
            q_s_o[ob_enc,option] = q_s_o[ob_enc,option] + alpha*(reward + (gamma**j)*(np.amax(q_s_o[next_ob_enc,:])) - q_s_o[ob_enc,option])
            # print("updated")
            ob = next_ob
            ob_enc = next_ob_enc
        steps.append(k)
